...
<div id="comments" class="comments-area boxed">
	...
	<?php if ( have_comments() ) : ?>
		...
		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
			<nav role="navigation" id="comment-nav-above" class="site-navigation comment-navigation">
				<h1 class="assistive-text"><?php esc_html_e( 'Comment navigation', 'upright' ); ?></h1>
				<div
					class="nav-previous"><?php previous_comments_link( esc_html__( '&larr; Older Comments', 'upright' ) ); ?></div>
				<div class="nav-next"><?php next_comments_link( esc_html__( 'Newer Comments &rarr;', 'upright' ) ); ?></div>
	...
	if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
		?>
		<p class="nocomments"><?php esc_html_e( 'Comments are closed.', 'upright' ); ?></p>
	<?php endif; ?>
	...
	if ( comments_open() ) :
		...
		comment_form( array( 'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
		                     ...
		                     'comment_notes_after'  => '<p class="required-attr meta">' . esc_html__( '(*) Required, Your email will not be published', 'upright' ) . '</p>',
		                     'respond_title'        => '<h3 id="reply-title" class="section-title">' . esc_html__( 'Leave a comment', 'upright' ) . '</h3>'
		) );
