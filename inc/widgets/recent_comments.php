...
class Upright_Widget_Recent_Comments extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'upright_recent_comments', esc_html__( 'Upright - Recent Comments', 'upright' ), array(
			'classname'   => 'widget_comments_wrap',
			'description' => esc_html__( 'Display most recent comments.', 'upright' )
		) );
	}

	public function widget( $args, $instance ) {
		$default  = array(
			'title'    => esc_html__( 'Latest Comments', 'upright' ),
			'quantity' => 5
		);
				...
				<article>
					<header class="clearfix">
						...
						<div class="entry-meta"><a
								...
								class="comment_link"><?php echo $comment->comment_author; ?></a> <?php esc_html_e( 'commented on', 'upright' ); ?>
						</div>
	...
	public function form( $instance ) {
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Widget title:', 'upright' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'quantity' ) ); ?>"><?php esc_html_e( 'Number of Comments:', 'upright' ); ?></label><br/>
