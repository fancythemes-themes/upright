...
class Upright_Widget_Recent_Posts extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'upright_recent_posts', esc_html__( 'Upright - Recent Posts', 'upright' ), array(
			'classname'   => 'widget_posts_wrap',
			'description' => esc_html__( 'Display most recent Posts.', 'upright' )
		) );
	}

	public function widget( $args, $instance ) {
		$default  = array(
			'title'    => esc_html__( 'Recent Posts', 'upright' ),
			'cats'     => '',
			'cat'      => '',
			'quantity' => 5,
			'order'    => 'date',
	...
	public function form( $instance ) {
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Widget title (Will automaticly use category name when select single category):', 'upright' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Category:', 'upright' ); ?></label><br/>
			<select id="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"
			        name="<?php echo esc_attr( $this->get_field_name( 'cat' ) ); ?>">
				<option
					value="0" <?php selected( 0, $cat ); ?>><?php esc_html_e( 'Multiple Categories', 'upright' ); ?></option>
				<?php
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'cats' ) ); ?>"><?php esc_html_e( 'Enter ID of categories e.g. 1,2,3,4. Leave it blank to pull all categories (if multiple category choosed).:', 'upright' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'quantity' ) ); ?>"><?php esc_html_e( 'Number of posts:', 'upright' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php esc_html_e( 'Order posts by:', 'upright' ); ?></label><br/>
			<select id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"
			        name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>">
				<option
					value="date" <?php selected( 'date', $order ); ?>><?php esc_html_e( 'date', 'upright' ); ?></option>
				<option
					value="rand" <?php selected( 'rand', $order ); ?>><?php esc_html_e( 'random', 'upright' ); ?></option>
				<option
					value="comment_count" <?php selected( 'comment_count', $order ); ?>><?php esc_html_e( 'popular', 'upright' ); ?></option>
			</select>
		</p>
