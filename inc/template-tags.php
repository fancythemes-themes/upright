	...
	function upright_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) :
			...
			case 'trackback' :
				...
				<p><?php esc_html_e( 'Pingback:', 'upright' ); ?><?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'upright' ), ' ' ); ?></p>
				<?php
				break;
			default :
				?>
				...
				<article id="comment-<?php comment_ID(); ?>" class="comment">
					<footer>
						<div class="comment-author vcard">
							...
							<?php printf( '%s <span class="says">' . esc_html__('says:', 'upright' ). '</span>', sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
						</div><!-- .comment-author .vcard -->
						<?php if ( $comment->comment_approved == '0' ) : ?>
							<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'upright' ); ?></em>
							<br/>
						<?php endif; ?>
						...
						<div class="comment-meta commentmetadata">
							<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
								<time datetime="<?php comment_time( 'c' ); ?>">
									...
									printf( esc_html__( '%1$s at %2$s', 'upright' ), get_comment_date(), get_comment_time() ); ?>
								</time>
							</a>,
							...
							<?php edit_comment_link( esc_html__( '(Edit)', 'upright' ), ' ' );
							?>
...
function upright_carousel_posts( $args = '' ) {
	...
	<div id="carousel-slider" class="flexslider">
		<h3 class="section-title"><span><?php echo esc_html( upright_get_option( 'carousel_title', esc_html__( 'Headlines', 'upright' ) ) ); ?></span></h3>
...
function upright_related_posts() {
	...
	if ( $related_posts ) {
		...
		<li class="no_related_post type-3"><?php esc_html_e( 'Cannot Retrieved a Related Posts Yet!', 'upright' ); ?></li><?php
	}
...
function upright_breadcrumb() {
	if ( ! is_front_page() ) {
		echo '<div id="breadcrumbs"> <a href="' . esc_url( home_url() ) . '">' . esc_html__( 'Home', 'upright' ) . '</a> ';
	}
	...
	if ( is_tag() ) {
		printf(  esc_html__( 'Tag: %s', 'upright' ), single_tag_title( '', false ) );
	}
	if ( is_404() ) {
	...
	if ( is_author() ) {
		printf( esc_html__( 'Posts by %s', 'upright' ), get_the_author() );
	}
	...
	if ( $total > 1 ) {
		...
		if ( strpos( $paginate[0], 'prev' ) !== false ) {
			...
			if ( ( $current_page - $show_number ) > 1 ) {
				...
				$first    = '<li>' . preg_replace( '/>[^>]*[^<]</', '>'. esc_html__( 'First', 'upright' ) .'<', $paginate[1] ) . '</li>';
				$left_dot = '<li><span>' . esc_html__( '...', 'upright' ) . '</span></li>';
			}
		}
		...
		if ( strpos( $paginate[ count( $paginate ) - 1 ], 'next' ) !== false ) {
			...
			if ( ( $current_page + $show_number ) < $total ) {
				...
				$last      = '<li>' . preg_replace( '/>[^>]*[^<]</', '>' . esc_html__( 'Last', 'upright' ) . '<', $paginate[ count( $paginate ) - 2 ] ) . '</li>';
				$right_dot = '<li><span>' . esc_html__( '...', 'upright' ) . '</span></li>';
			}
		}

		/* translators: 1. Current Page No 2. Total Pages Count */
		echo '<span class="page-of">'. sprintf( esc_html__('Page %1$s of %2$s', 'upright'), $current_page, $total ) . '</span>';
		...
		echo '<nav class="page-navigation">';
		echo '<span class="page-of">' . esc_html__( 'Page 1 of 1', 'upright' ) . '</span>';
		echo '</nav>';
	}
}