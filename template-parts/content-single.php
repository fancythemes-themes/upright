...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'group' ); ?>>
	<header class="entry-header">
		<div class="entry-meta-single">
			...
			if ( $categories_list && upright_categorized_blog() ) {
				printf( '<span class="inline-icon-ribbon">%1$s</span>', $categories_list );
			}
			?>
			<span
				class="inline-icon-comment"><?php comments_popup_link( esc_html__( 'Leave a comment', 'upright' ), esc_html__( '1 Comment', 'upright' ), esc_html__( '% Comments', 'upright' ) ); ?></span>

			<?php edit_post_link( esc_html__( 'Edit', 'upright' ), '<span class="inline-icon-pencil">', '</span>' ); ?>
		</div>
	...
	<div class="entry-content boxed">
		<?php the_content(); ?>
		<p class="post-tags"><?php the_tags( '<span class="inline-icon-tag hidden-text-icon">' . esc_html__( 'Tags:', 'upright' ) . '</span> ', ', ', '' ); ?></p>
		<?php wp_link_pages( array(
			'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'upright' ),
			'after'  => '</div>'
		) ); ?>
	</div>
